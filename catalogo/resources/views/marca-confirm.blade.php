...
<main>
    <div class="mx-auto max-w-4xl py-12 px-8">
        ...
        <div class="text-yellow-400 p-4 col-6 mx-auto shadow ">
            Se eliminará la marca:
            <span class="block py-2.5 px-0 w-full text-2xl bg-transparent border-0 border-b-2 appearance-none text-teal-400 border-gray-600 focus:outline-none focus:ring-0 focus:border-teal-600">
                {{ $marca->mkNombre }}
            </span>
            <form action="/marca/{{ $marca->idMarca }}/destroy" method="post">
                @csrf
                @method('delete')
